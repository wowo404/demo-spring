 ...
 * 
 * File Created at 2013年12月19日 下午3:03:15
 * 
...
public class JedisTest extends TestCase {
	...
	public void testBasicString() {
		...
		//改
		jedis.append("name", "gogogo");
		System.out.println(jedis.get("name"));
		//改-直接覆盖
		jedis.set("name", "xiaogugu");
		System.out.println(jedis.get("name"));
		//删
		jedis.del("name");
		System.out.println(jedis.get("name"));
		 ...
		 */
		Long num = jedis.msetnx("type", "wo", "age", "20");
		System.out.println(num);
		System.out.println(jedis.mget("type", "age"));
	}
	...
	public void testMap() {
		...
		user.put("title", "how to learn java");
		user.put("length", "100000");
		jedis.hmset("user", user);
		//取出user中的name，执行结果:[minxr]-->注意结果是一个泛型的List
		//第一个参数是存入redis中map对象的key，后面跟的是放入map中的对象的key，后面的key可以跟多个，是可变参数
		List<String> result = jedis.hmget("user", "title");
		System.out.println(result);

		//删除map中的某个键值
		//jedis.hdel("user", "length");
		System.out.println(jedis.hmget("user", "length"));//因为删除了，所以返回的是null
